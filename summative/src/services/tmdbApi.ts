...
export const getMovies = (category: string, page: number = 1) =>
  api.get<ApiResponse<Movie>>(`/movie/${category}`, { params: { page } }).then(r => r.data);

export const getMovieDetails = (id: number) =>
  api.get<MediaDetails>(`/movie/${id}`).then(r => r.data);

export const getTVShows = (category: string, page: number = 1) =>
  api.get<ApiResponse<TVShow>>(`/tv/${category}`, { params: { page } }).then(r => r.data);

export const getTVDetails = (id: number) =>
  api.get<MediaDetails>(`/tv/${id}`).then(r => r.data);

export const getTrending = (mediaType: 'movie' | 'tv' | 'all', timeWindow: 'day' | 'week', page: number = 1) =>
  api.get<ApiResponse<Movie | TVShow>>(`/trending/${mediaType}/${timeWindow}`, { params: { page } }).then(r => r.data);
...
export const getVideos = (mediaType: 'movie' | 'tv', id: number) =>
  api.get<{ results: Video[] }>(`/${mediaType}/${id}/videos`).then(r => r.data.results);

export const getReviews = (mediaType: 'movie' | 'tv', id: number, page: number = 1) =>
  api.get<ApiResponse<Review>>(`/${mediaType}/${id}/reviews`, { params: { page } }).then(r => r.data);
...
export const getSeasonDetails = (tvId: number, seasonNumber: number) =>
  api.get<{ episodes: Episode[]; name: string; overview: string; poster_path: string | null }>(
...
export const getPersonCredits = (id: number) =>
  api.get<{ cast: CareerRole[]; crew: CareerRole[] }>(`/person/${id}/combined_credits`).then(r => r.data);

export const getPersonImages = (id: number) =>
  api.get<{ profiles: PersonImage[] }>(`/person/${id}/images`).then(r => r.data.profiles);
