import { FaArrowRight, FaFilm, FaFire, FaStar, FaTv } from "react-icons/fa";
...
export default function HomeView() {
	...
	} = useFetch(() => getMovies("now_playing"));
	const {
		data: tvShows,
		loading: tvLoading,
		error: tvError,
		refetch: refetchTV,
	} = useFetch(() => getTVShows("airing_today"));
	const {
		data: trending,
		loading: trendingLoading,
		error: trendingError,
		refetch: refetchTrending,
	} = useFetch(() => getTrending("all", "week"));
		...
		<div className="space-y-12">
			...
			<section>
				<div className="flex items-center justify-between mb-6">
					...
					<Link
						to="/movies"
						className="text-tmdb-light hover:underline flex items-center gap-1"
					>
				...
				{moviesError && (
					<ErrorMessage message={moviesError} onRetry={refetchMovies} />
				)}
				{movies && (
