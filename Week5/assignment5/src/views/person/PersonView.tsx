...
export default function PersonView() {
  ...
  const { data: person, loading: personLoading, error: personError, refetch: refetchPerson } = useFetch(
    () => getPersonDetails(numericId),
    [numericId]
  );
  const { data: credits, loading: creditsLoading, error: creditsError, refetch: refetchCredits } = useFetch(
    () => getPersonCredits(numericId),
    [numericId]
  );

  if (personLoading || creditsLoading) return <Loading />;
  if (personError) return <ErrorMessage message={personError} onRetry={refetchPerson} />;
  if (creditsError) return <ErrorMessage message={creditsError} onRetry={refetchCredits} />;
        ...
        <div className="flex-1 space-y-4">
          ...
          {person.also_known_as.length > 0 && (
            <div>
              ...
              <p className="text-sm text-gray-300">{person.also_known_as.join(', ')}</p>
            </div>
          )}
